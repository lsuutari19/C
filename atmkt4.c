...
int phonePlanCharge(int savings);
...
int functionHandle(int checkings) {
    ...
    // nosto ja liittymanlataus 
    int mainChoice, withdrawn, chargedphone;
    int giveMoney[3] = {-1, 0};
    do{
        ...
        {
            chargedphone = phonePlanCharge(checkings);
            if(chargedphone >= 10){
                checkings = checkings - chargedphone;
                mainChoice = 102;
            }
            printf("charged phone is %d", chargedphone);
...
int mainScreen() {
    ...
    {
        printf("\n\n            Valitse yksi: \n");
        printf("\n\n1.Otto  2.Saldo  3.Tapahtumat  4.Liittymanlataus 5. Lopeta\n");
        printf("\n\n        > ");
        if(scanf("%d", &choice) == 1) {
            continue;
        } else {
...
int phonePlanCharge(int savings) {
            ...
            int phoneCarrier=99, phonePlan = 0;
            do
            {
                printf("\n\n\n        Liittyman lataus\n");
...
// tyhjentää bufferin jos käyttäjän input on ollut väärää tyyppiä
void clearBuffer()
{
    char c;
    while ( (c = getchar()) != '\n' && c != EOF);
